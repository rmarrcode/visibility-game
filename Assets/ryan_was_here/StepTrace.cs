...
public class StepTrace
{
    ...
    public float[] GetSurroundingSteps(int x, int z)
    {
        ...
        vision[0] = InBounds(x-1, z-1) ? steps[x-1,z-1] : 0;
        vision[1] = InBounds(x-1, z) ? steps[x-1,z] : 0;
        vision[2] = InBounds(x-1, z+1) ? steps[x-1,z+1] : 0;
        vision[3] = InBounds(x, z-1) ? steps[x,z-1] : 0;
        vision[4] = InBounds(x, z+1) ? steps[x,z+1] : 0;
        vision[5] = InBounds(x+1, z-1) ? steps[x+1,z-1] : 0;
        vision[6] = InBounds(x+1, z) ? steps[x+1,z] : 0;
        vision[7] = InBounds(x+1, z+1) ? steps[x+1,z+1] : 0;
        return vision;
    }
